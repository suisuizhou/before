...
def get_model(
    ...
    if net_F is None:
        assert model_name in ['ResNet18', 'ViT1D', 'EverAdaptCNN1D', 'ResNet18_1D_SDE']
        model_ = getattr(Model_Zoos, model_name)

        # ResNet18 uses internal channel configuration by default.
        # Other models keep the original cfg passing behavior.
        if model_name == "ResNet18":
            # Pass an optional channel configuration for structured pruning.
            cfg_arg = cfg
            if cfg is not None and hasattr(cfg, "Model"):
                cfg_arg = getattr(cfg.Model, "prune_cfg", None)
            netF = model_(cfg=cfg_arg) if cfg_arg is not None else model_()
        else:
            netF = model_(cfg=cfg)
