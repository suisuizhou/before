...
Differences from ``main_tta_0711_full_tuned.py``:
  * target stream is visited exactly once and in deterministic dataset order;
...
class Strict0711ResNetTrainer(EVMT0711Trainer):
    ...
    def setup(self):
        super().setup()
        # Replace the shuffled adaptation loader created by the tuned version.
        self.stream_seed = int(
            _get(self.tcfg, "stream_seed", self.cfg.seed_run)
        )
    ...
    def adapt(self):
        ...
        print(
            f"[STRICT 0711] batches={total_batches}, passes=1, shuffle=False, "
