...
class EvidenceMemoryBank:
    ...
    def covered_classes(self) -> int:
        return sum(1 for entries in self._features if len(entries) > 0)

    def coverage_ratio(self) -> float:
        if self.num_classes <= 0:
            return 0.0
        return float(self.covered_classes()) / float(self.num_classes)
...
# -----------------------------------------------------------------------------
# Full-mode activation schedule
# -----------------------------------------------------------------------------


def full_auxiliary_schedule(
    memory: Optional[EvidenceMemoryBank],
    current_pass: int,
    warmup_passes: int,
    ramp_passes: int,
    min_pcl_classes: int,
    ...
    """Gate PCL/NCL until the evidence-weighted bank is sufficiently mature.

    The memory is still populated during warm-up.  Auxiliary losses only become
    active after warm-up and are linearly ramped to their configured weights.
    """
    bank_size = len(memory) if memory is not None else 0
    bank_classes = memory.covered_classes() if memory is not None else 0

    if int(current_pass) <= int(warmup_passes):
        ramp = 0.0
    else:
        ramp_steps = max(1, int(ramp_passes))
        ramp = min(1.0, float(current_pass - warmup_passes) / float(ramp_steps))

    pcl_on = bool(
        ramp > 0.0
        and memory is not None
        and bank_classes >= int(min_pcl_classes)
    )
    ncl_on = bool(
        ramp > 0.0
        and memory is not None
        and bank_classes >= int(min_ncl_classes)
        and bank_size >= int(min_ncl_entries)
    )
    return {
        "ramp": ramp,
        "pcl_on": pcl_on,
...
class EVMT0711Trainer:
    ...
    def adapt(self):
        ...
        warmup_passes = int(_get(self.tcfg, "warmup_passes", 1))
        aux_ramp_passes = int(_get(self.tcfg, "aux_ramp_passes", 2))
        min_pcl_classes = int(_get(self.tcfg, "min_pcl_classes", 8))
        min_ncl_classes = int(_get(self.tcfg, "min_ncl_classes", 16))
        min_ncl_entries = int(_get(self.tcfg, "min_ncl_entries", 64))
        ...
        for iter_num in range(1, max_iter + 1):
            ...
            aux_schedule = full_auxiliary_schedule(
                memory=self.memory if mode == "full" else None,
                current_pass=current_pass,
                warmup_passes=warmup_passes,
                ramp_passes=aux_ramp_passes,
                min_pcl_classes=min_pcl_classes,
                min_ncl_classes=min_ncl_classes,
                min_ncl_entries=min_ncl_entries,
            )
            if mode == "full" and self.memory is not None:
                if aux_schedule["pcl_on"]:
            ...
            lambda_mt_eff = lambda_mt * (
                mt_warmup_scale if current_pass <= warmup_passes else 1.0
            )
            lambda_pcl_eff = lambda_pcl * aux_schedule["ramp"] if aux_schedule["pcl_on"] else 0.0
            lambda_ncl_eff = lambda_ncl * aux_schedule["ramp"] if aux_schedule["ncl_on"] else 0.0
            ...
            if should_log:
                ...
                print(
                    ...
                    f"bank={aux_schedule['bank_size']} | "
                    f"bank_cls={aux_schedule['bank_classes']}/{self.num_classes} | "
                    f"pcl_on={int(aux_schedule['pcl_on'])} | "
                    f"ncl_on={int(aux_schedule['ncl_on'])} | "
                    f"w_mt={lambda_mt_eff:.4f} | w_pcl={lambda_pcl_eff:.4f} | "
                    f"w_ncl={lambda_ncl_eff:.4f} | "
